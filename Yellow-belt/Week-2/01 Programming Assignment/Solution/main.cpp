...
ostream& operator << (ostream& os, const StopsForBusResponse& r) {
    if (r.stops_to_buses.empty()) {
        ...
        bool first = true;
        for (const auto& stop_buses : r.stops_to_buses) {
            if (!first) {
                os << endl;
            }
            first = false;
            os << "Stop " << stop_buses.first << ":";
            if (stop_buses.second.size() == 1) {
                os << " no interchange";
            } else {
                for (const auto& bus : stop_buses.second) {
                    if (bus != r.bus) {
                        os << " " << bus;
                    }
                }
...
ostream& operator << (ostream& os, const AllBusesResponse& r) {
    if (r.buses_to_stops.empty()) {
        ...
        bool first = true;
        for (const auto& bus_stops : r.buses_to_stops) {
            if (!first) {
                os << endl;
            }
            first = false;
