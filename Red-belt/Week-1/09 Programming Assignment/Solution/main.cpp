...
public:
    Paginator(Iterator first, Iterator last, size_t page_size) {
        for (size_t remaining = distance(first, last); remaining > 0; ) {
            size_t current = min(page_size, remaining);
            Iterator page_end = next(first, current);
            pages.push_back({first, page_end});
...
template <typename C>
auto Paginate(C& container, size_t page_size) {
